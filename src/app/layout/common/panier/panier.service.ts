...
export class PanierService {
  ...
  loadPanier(): void {
    this._httpClient.get<any[]>(`${this._apiUrl}panier/current`).subscribe(
  ...
  removeFromCart(articleId: number): Observable<any> {
    return this._httpClient
      .delete(`${this._apiUrl}panier/remove/${articleId}`)
      .pipe(tap(() => this.loadPanier()));
  }

  updateQuantity(articleId: number, quantity: number): Observable<any> {
    return this._httpClient
      .put(`${this._apiUrl}panier/update`, { articleId, quantity })
      .pipe(tap(() => this.loadPanier()));
  }

  checkout(): Observable<any> {
    return this._httpClient
      .post(`${this._apiUrl}panier/checkout`, {})
      .pipe(tap(() => this.loadPanier()));
  }

  addToCart(articleId: number, quantity: number): Observable<any> {
    return this._httpClient
      .post(`${this._apiUrl}panier/add-to-cart`, null, {
        params: {
